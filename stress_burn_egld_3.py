...
Identique à stress_burn_egld_2.py + gas price dynamique :
  - wallet riche (>= RICH_THRESHOLD) → gas price premium (+20%) pour priorité mempool
  - wallet pauvre                     → gas price minimum pour préserver les derniers fonds
...
RICH_THRESHOLD    = int(0.1  * 1e18)   # 0.1 EGLD  → passe en mode redistribution
RICH_VALUE        = int(0.01 * 1e18)   # 0.01 EGLD par tx (= ~200 tx de gas pour le récepteur)
PREMIUM_GAS_PRICE = int(config.DEFAULT_GAS_PRICE * 1.2)  # +20% → priorité mempool si wallet riche
...
def wallet_worker(
    ...
    empty_streak = 0  # backoff exponentiel quand le wallet est vide
    ...
    while True:
        ...
        # 2. Effective value + gas price : redistribution rapide et priorité mempool si wallet riche
        effective_value     = RICH_VALUE        if balance >= RICH_THRESHOLD else amount_per_tx
        effective_gas_price = PREMIUM_GAS_PRICE if balance >= RICH_THRESHOLD else config.DEFAULT_GAS_PRICE
        ...
        if available_balance == 0 or available_balance < GAS_COST_RAW:
            wait = min(1.0 * (2 ** empty_streak), 30.0)  # 1s → 2s → 4s … 30s max
            empty_streak += 1
            time.sleep(wait)
            continue

        empty_streak = 0  # reçu des fonds → reset du backoff
        ...
        # 7. Dry-run : afficher le batch et arrêter
        if dry_run:
            for tx in batch:
...
def main():
    parser = argparse.ArgumentParser(description="Burn EGLD Stress Test — intra-shard bulk batch + gas price dynamique")
    ...
    print(f"[INFO] Gas price: {config.DEFAULT_GAS_PRICE} (pauvre) / {PREMIUM_GAS_PRICE} (riche ≥ {RICH_THRESHOLD / 1e18} EGLD)")
    if args.dry_run:
