...
v2 — correctif : nonce incrémenté APRÈS send_transaction (et seulement si succès)
     pour éviter les gaps de nonce en cas d'erreur réseau.

Usage:
  # Affichage seul
  python check_balances_2.py \
    --wallets-dir ./wallets/shard-0 \
    --wallets-dir ./wallets/shard-1 \
    --wallets-dir ./wallets/shard-2 \
    --max-wallets 500

  # Top-up automatique EGLD
  python check_balances_2.py \
...
def main():
    ...
    for r in results:
        ...
        if min_egld_raw is not None and r["egld"] < min_egld_raw:
            ...
            topup_count += 1
            if args.dry_run:
                nonce += 1   # dry-run : preview des nonces
            else:
                try:
                    provider.send_transaction(tx)
                    nonce += 1   # FIX : nonce avancé uniquement si send réussi
                except Exception as e:
                    print(f"[WARN] EGLD top-up failed for {r['name']}: {e}")
                    topup_failed += 1
                    # nonce NON incrémenté → prochain top-up réessaie avec le même nonce
    ...
    if topup_failed:
        summary += f"  ({topup_failed} échoués)"
    print(f"\n{tag} {summary}")
